...
@aioctl.aiotask
async def test(a, b=1, alog=log):
    await asyncio.sleep_ms(100)
    alog.info(f"[test_{a}] start: hello {a}, {b}")
    t0 = time.time()
    while (time.time() - t0) < 5:
        await asyncio.sleep(2)
    alog.info(f"[test_{a}] done: hello {a}, {b}")
    return 42
...
async def main():
    ...
    aioctl.add(test, "from", b=20, name="test_from", _id="test_from")
