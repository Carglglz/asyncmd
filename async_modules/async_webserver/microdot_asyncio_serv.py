...
class Microdot(BaseMicrodot):
    async def start_server(
        self, host="0.0.0.0", port=5000, debug=False, ssl=None, tasks=[]
    ):
    ...
    def set_config(self, name, log, request_counter):
        self.webserver_name = name
        self.log = log
    ...
    async def shutdown(self):
        async def delayed_shutdown(t):
            await asyncio.sleep(t)
            self.server.close()

        asyncio.create_task(delayed_shutdown(0.5))

    async def handle_request(self, reader, writer):
        # print("Handling request now...")
        req = None
        try:
            # print("Request create now")
