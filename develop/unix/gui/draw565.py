...
class Draw565(object):
    ...
    def fill(self, bg=None, x=0, y=0, w=None, h=None):
        ...
        """
        self._display.rfill(bg, x, y, w, h)

    @micropython.native
    def blit(self, image, x, y, fg=0xFFFF, c1=0x4A69, c2=0x7BEF, reverse=False):
        ...
        if len(image) == 3:
            # Legacy 1-bit image
            self.rleblit(image, (x, y), fg, reverse=reverse)
        else:  # elif image[0] == 2:
            # 2-bit RLE image, (255x255, v1)
            self._rle2bit(image, x, y, fg, c1, c2)

        self._display.refresh_window()

    @micropython.native
    def rleblit(self, image, pos=(0, 0), fg=0xFFFF, bg=0, reverse=False):
        ...
        display.set_window(pos[0], pos[1], sx, sy, reverse=reverse)
        # time.sleep(0.5)
