        ...
        if testdir:
            ...
            if log:
                ...
                if all(t.wasSuccessful() for t in self.test_result.values()):
                    ...
                    if not rollback_ota_on_error:
                        return self._FAIL

                    self.log.info(f"[{self.name}.service] Rolling back partion...")
                    print(f"[{self.name}.service] unittest core failed...")
                    print(f"[{self.name}.service] Rolling back partion...")
                    try:
                        import machine

                        await asyncio.sleep_ms(500)
                        machine.reset()
                        # with OTA firmware this is enough
                        # with filesystem scripts/code --> custom rollback
                        # function
                        # WARNING: be careful of a potential reset loop if
                        # updating tests that fail.
                    except Exception:
                        return self._FAIL


service = UnittestService("unittest_core")
