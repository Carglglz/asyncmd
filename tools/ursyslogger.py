...
class RsysLogger:
    ...
    def __init__(self, addr, port=514, hostname=None, facility=LOG_USER,
                 msg_as_utf8=True, t_offset="+00:00", tls=False, tls_params={}):
    ...
    def connect(self, tls=False, tls_params={}):
        ...
        self.connected = True
        if tls:
            if tls_params["keyfile"]:
                with open(tls_params["keyfile"], "rb") as kf:
                    tls_params["keyfile"] = kf.read()
            if tls_params["certfile"]:
                with open(tls_params["certfile"], "rb") as cf:
                    tls_params["certfile"] = cf.read()
            if tls_params["cadata"]:
                with open(tls_params["cadata"], "rb") as cdf:
                    tls_params["cadata"] = cdf.read()
            self.sock = ssl.wrap_socket(self.sock, **tls_params)
